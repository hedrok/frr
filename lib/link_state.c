			       ...
			       bool verbose)
{
	...
	if (CHECK_FLAG(lsn->flags, LS_NODE_ISIS_AREA_ID)) {
		sbuf_push(&sbuf, 4, "IS-IS Area ID: ");
		for (int i = 0; i < lsn->isis_area_id_len; i++) {
			sbuf_push(&sbuf, 0, (i % 2 ? "%02x." : "%02x"), lsn->isis_area_id[i]);
		}
		sbuf_push(&sbuf, 0, "\n");
	}
				...
				struct json_object *json)
{
	...
	if (CHECK_FLAG(lsn->flags, LS_NODE_ISIS_AREA_ID)) {
		...
		json_object_string_addf(json, "isis-area-id",
					"%pIS", &area_addr);
	}
}
