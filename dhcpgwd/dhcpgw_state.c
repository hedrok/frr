...
int dhcpgw_state_save(const char* ifname, bool up, int af, const union g_addr *ip_addr)
{
    ...
    if (!up) { /*down*/
        ...
        /* This directory can be created by tmpfiles.d with proper owner */
...
int dhcpgw_state_read(const char* ifname, bool *up, int *af, union g_addr *ip_addr)
{
    ...
    if (!fd) {
        zlog_warn("%s: couldn't open file %s for reading, error: %s", __func__, path, strerror(errno));
        return -errno;
    }

    if (!fgets(ip_str, sizeof(ip_str), fd)) {
        zlog_warn("%s: couldn't read data from file %s, error: %s", __func__, path, strerror(errno));
        fclose(fd);
        return -errno;
    }
    fclose(fd);
