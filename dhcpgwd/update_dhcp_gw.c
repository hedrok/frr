 ...
 *
 * For this to work either DHCP clients should be run as root,
 * or update-dhcp-gw should be owned by frr user and have setuid
 * mode. 
 *
 * systemd-tmpfiles creates directory for state files and sets ownership
 * and setuid mode for this to work if configured with
 * tools/etc/tmpfiles.d/frr-dhcpgw.conf.in
 *
...
static void help(const char *command_name)
{
    fprintf(stderr, "Usage: %s interface [up IP|down]\n"
...
int main(int argc, char **argv)
{
     ...
     *
     * But special symbols are not processed yet
     */
    ...
    /* All is ok, time to call 'dhcp-gateways-update' */
