...
static int dhcpgw_route_nb_run(struct vty *vty, struct dhcpgw_route_args *args)
{
    // DEBUG
	zlog_notice("HEDROK dhcpgw_route_nb_run, no: %d, argc: %d", args->no, args->argc);
    int i;
    for (i = 0; i < args->argc; i++) {
        zlog_notice("...%d: %s", i, args->argv[i]->arg);
    }
	zlog_notice("...all");

    int dhcpgw_index = 0;
    if (!argv_find(args->argv, args->argc, DHCP_GATEWAY_CMD_STR, &dhcpgw_index)) {
        zlog_err("Unexpected command in dhcpgw_route_nb_run! " DHCP_GATEWAY_CMD_STR " should have been found!");
        return 1;
    }
    char *prefix = argv_concat(args->argv, dhcpgw_index, args->no ? 1 : 0);
    char *suffix = argv_concat(args->argv, args->argc, dhcpgw_index + 1);

	zlog_notice("prefix: %s", prefix);
	zlog_notice("suffix: %s", suffix);
...
const struct frr_yang_module_info frr_dhcpgwd_cli_info = {
	.name = "frr-dhcpgwd",
	.ignore_cfg_cbs = true,
	.nodes = {
		{
...
DEFPY_YANG(debug_dhcpgwd, debug_dhcpgwd_cmd,
	   "[no] debug dhcpgw",
	   NO_STR DEBUG_STR DHCPGWD_STR
	   )
{
	dhcpgw_debug_set(vty->node, !no);
	zlog_notice("HEDROK was in debug_dhcpgwd_cmd");
 ...
 */
DEFPY_YANG(ip_route_dhcpgw_interface,
...
{
    zlog_notice("HEDROK: ip_route_dhcpgw_interface_cmd, destination ip: %s; interface: %s", prefix, ifname);
    ...
    dhcpgw_route_nb_run(vty, &args);
	return CMD_SUCCESS;
}

DEFPY_YANG(dhcpgw_update,
      dhcpgw_update_cmd,
...
{
    zlog_notice("HEDROK: dhcpgw_update_cmd, interface: %s", ifname);
    dhcpgw_routes_update_interface(ifname);
	return CMD_SUCCESS;
}

DEFPY_YANG(dhcpgw_show_route,
