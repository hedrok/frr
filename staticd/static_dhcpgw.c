...
static int cmp_static_dhcpgw_nexthop(
	const struct static_dhcpgw_nexthop *a,
...
/*
 * List of interface lease files to watch
 */
PREDECL_SORTLIST_UNIQ(static_dhcpgw_interface_list);

/* List of interfaces with cached data to watch */
...
static int cmp_static_dhcpgw_interface(
	const struct static_dhcpgw_interface *a,
 ...
 */
static void get_dhcpgw_dhclient_ipv4_address(struct in_addr *out_addr, bool *out_valid, const char* lease_path);

/*
 * Lease files cache functions
 *
 * We store list of static_dhcpgw_interface, one per each interface
 * that is used in any of watched nexthops.
 *
 * Each list item stores path, last modification date, last address
 * and last valid. Also it stores counter of nexthops with the interface
...
static void static_dhcpgw_interface_init(struct static_dhcpgw_interface* pif, struct static_nexthop *nh);
...
static void static_dhcpgw_timer_event_cb(struct event *);
...
extern void static_dhcpgw_add_nexthop_watch(struct static_nexthop *nh)
{
	...
	dgnh.nh = nh;
	// Check if there is this nexthop already not
	// to do extra alloc
	if (!static_dhcpgw_nexthop_list_find(&nexthops_list_head, &dgnh)) {
		struct static_dhcpgw_nexthop* p = XCALLOC(
			MTYPE_STATIC_DHCPGW_NEXTHOP,
			sizeof(struct static_dhcpgw_nexthop));
		p->nh = nh;
		static_dhcpgw_nexthop_list_add(
			&nexthops_list_head,
			p);
...
extern void static_dhcpgw_del_nexthop_watch(struct static_nexthop *nh)
{
	...
	struct static_dhcpgw_nexthop* p =
		static_dhcpgw_nexthop_list_find(
			&nexthops_list_head,
			&dgnh);
	if (!p) {
		return;
	}
	static_dhcpgw_nexthop_list_del(
		&nexthops_list_head,
		&dgnh);
...
static void static_dhcpgw_update_nexthop(struct static_nexthop *nh, bool install, bool force_update)
{
	...
	if (!pif) {
	    zlog_warn("static_dhcpgw_update_nexthop couldn't get interface data for interface %s", nh->ifname);
		return;
	}
...
static void get_dhcpgw_dhclient_ipv4_address(struct in_addr *out_addr, bool *out_valid, const char* leasepath)
{
...
#define DHCLIENT_LEASE_NEW_ROUTERS_PREFIX "new_routers='"
	...
	*out_valid = false;
	...
	if (!fp) {
		zlog_warn("get_dhcpgw_dhclient_ipv4_address, couldn't open lease file %s", leasepath);
		return;
	}
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (strncmp(
				buf,
				DHCLIENT_LEASE_NEW_ROUTERS_PREFIX,
	...
	if (!ip) {
		zlog_warn("get_dhcpgw_dhclient_ipv4_address couldn't find ip in lease file %s", leasepath);
		return;
	}
	...
	if (inet_pton(AF_INET, ip, out_addr) != 1) {
		zlog_warn("get_dhcpgw_dhclient_ipv4_address couldn't parse ip '%s' with inet_pton, from lease file '%s'", ip, leasepath);
		return;
	}
...
static void static_dhcpgw_interface_update(struct static_dhcpgw_interface* pif)
{
	...
	if (stat(pif->lease_path, &statbuf)) {
		zlog_warn("static_dhcpgw_interface_update calling stat for %s failed", pif->lease_path);
		return;
	}
...
static void static_dhcpgw_interfaces_on_add_nexthop(struct static_nexthop *nh)
{
	...
	if (!pif) {
		pif = XCALLOC(
			MTYPE_STATIC_DHCPGW_INTERFACE,
			sizeof(struct static_dhcpgw_interface));

		static_dhcpgw_interface_init(pif, nh);

		static_dhcpgw_interface_list_add(
			&interfaces_list_head,
			pif);
	}
	pif->nh_count++;
}
...
static void static_dhcpgw_interfaces_on_del_nexthop(struct static_nexthop *nh)
{
	struct static_dhcpgw_interface* pif = get_dhcpgw_interface_by_ifname(nh->ifname);
	if (!pif) {
	    zlog_warn("static_dhcpgw_interfaces_on_del_nexthop couldn't get interface data for interface %s", nh->ifname);
		return;
	}
	pif->nh_count--;
	if (!pif->nh_count) {
		static_dhcpgw_interface_list_del(
		        &interfaces_list_head,
		        pif);
...
static void static_dhcpgw_start_timer_if_needed(void)
{
	if (poll_period_seconds
	       && !dhcpgw_timer
	       && static_dhcpgw_nexthop_list_first(&nexthops_list_head)) {
		event_add_timer(master, static_dhcpgw_timer_event_cb, NULL,
				poll_period_seconds, &dhcpgw_timer);
	}
}
